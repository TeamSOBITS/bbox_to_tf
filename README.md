...
- `enable_id`:\
  `true` にすると，TFフレーム名およびオブジェクトIDに検出インデックスを付与します（例: `mug_0`, `mug_1`）．\
  同一クラスの物体が複数存在する場合に有効です．\
  `false` の場合はクラス名のみを使用します（例: `mug`）．

- `enable_auto_rotation`:\
  `true` にすると，点群の重心とバウンディングボックスの各軸方向の広がりを比較し，物体の向きを自動推定してTFの回転に反映します．\
  X方向の広がりがZ方向より大きい場合はY軸方向に90°回転，Y方向の広がりがZ方向より大きい場合はX軸に-90°・Y軸に90°回転します．\
  `false` の場合は回転なし（identity）となります．
