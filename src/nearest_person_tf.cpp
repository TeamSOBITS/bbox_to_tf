...
class BboxToTF {
        ...
        void callback_BBoxCloud(const std::shared_ptr<sobits_msgs::msg::BoundingBoxes> bbox_msg,
            ...
            if (!execute_flag_) {
                ...
                for (size_t i=0; i<bbox_msg->bounding_boxes.size(); i++) {
                    ...
                    float person_distance = sqrt(pow(pt.x, 2) + pow(pt.y, 2));
    ...
    public:
        BboxToTF(std::shared_ptr<rclcpp::Node> nd) : nd_(nd), tfBuffer_(std::make_shared<rclcpp::Clock>(RCL_ROS_TIME)), tfListener_(tfBuffer_), tfBroadcaster_(nd_) {
            ...
            nd_->declare_parameter("only_specific_object", true);
            ...
            only_specific_object_ = nd_->get_parameter("only_specific_object").as_bool();
