...
class BboxTo3D : public rclcpp::Node {
  private:
    ...
    void processBBoxFastShot(const std::shared_ptr<vision_msgs::msg::Detection2DArray> bbox_msg,
      ...
      for (size_t i = 0; i < bbox_msg->detections.size(); i++) {
        ...
        if (original_index >= 0 && original_index < static_cast<int>(cloud_transformed_->points.size())) 
          tf_published = publishObjectTf(cloud_transformed_->points[original_index], object_id);
        ...
        if (tf_published) {
          ...
          for (const auto &center : sub_centers) {
            ...
            if (sub_index >= 0 && sub_index < static_cast<int>(cloud_transformed_->points.size())) 
              if (publishObjectTf(cloud_transformed_->points[sub_index], object_id)) {
                original_index = sub_index;
                break;
            }
          }
        }
        ...
        ohwp.hypothesis.class_id = bbox_msg->detections[i].results[0].hypothesis.class_id;
        ohwp.hypothesis.score = bbox_msg->detections[i].results[0].hypothesis.score;
        ohwp.pose.pose.position.x = cloud_transformed_->points[original_index].x;
        ohwp.pose.pose.position.y = cloud_transformed_->points[original_index].y;
        ohwp.pose.pose.position.z = cloud_transformed_->points[original_index].z;
        ohwp.pose.pose.orientation.x = 0.;
        ohwp.pose.pose.orientation.y = 0.;
        ohwp.pose.pose.orientation.z = 0.;
        ohwp.pose.pose.orientation.w = 1.;
        ohwp.pose.covariance = bbox_msg->detections[i].results[0].pose.covariance;
        object_pose.results.push_back(ohwp);
        object_pose.bbox.center.position.x = cloud_transformed_->points[original_index].x;
        object_pose.bbox.center.position.y = cloud_transformed_->points[original_index].y;
        object_pose.bbox.center.position.z = cloud_transformed_->points[original_index].z;
        ...
        object_pose.bbox.size.x = 2*noise_point_cloud_range;
        object_pose.bbox.size.y = 2*noise_point_cloud_range;
        object_pose.bbox.size.z = 2*noise_point_cloud_range;
        object_pose.id = generateObjectId(bbox_msg->detections[i].id, i);
        object_pose_array.detections.push_back(object_pose);
        combined_cloud->points.push_back(cloud_transformed_->points[original_index]);
      }

      sensor_msgs::msg::PointCloud2 combined_cloud_msg;
      pcl::toROSMsg(*combined_cloud, combined_cloud_msg);
      combined_cloud_msg.header = pcl_msg->header;
    ...
    void processBBoxClustering(const std::shared_ptr<vision_msgs::msg::Detection2DArray> bbox_msg,
                                const std::shared_ptr<sensor_msgs::msg::PointCloud2> pcl_msg,
                                const std::shared_ptr<sensor_msgs::msg::Image> img_msg) {
      ...
      for (size_t i = 0; i < bbox_msg->detections.size(); i++) {
        ...
        if ((((int)(img_msg->width) * ((int)(bbox.bbox.center.position.y)) + (int)(bbox.bbox.center.position.x)) < 0) || ((int)(cloud_transformed_->points.size()) <= ((int)(img_msg->width) * ((int)(bbox.bbox.center.position.y)) + (int)(bbox.bbox.center.position.x))))
          continue;
        if (!checkNanInf(cloud_transformed_->points[(int)(img_msg->width) * ((int)(bbox.bbox.center.position.y)) + (int)(bbox.bbox.center.position.x)]))
          continue;
        ...
        if (cloud_bbox_xyz->points.size() == 0) continue;
        if (!checkNanInf(cloud_bbox_xyz->points[0])) continue;
        for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(), it_end = cluster_indices.end();
          ...
          double tmp_dis = std::sqrt(std::pow(((tmp_min_pt.x() + tmp_max_pt.x()) / 2.) - cloud_bbox_xyz->points[0].x, 2) + std::pow(((tmp_min_pt.y() + tmp_max_pt.y()) / 2.) - cloud_bbox_xyz->points[0].y, 2) + std::pow(((tmp_min_pt.y() + tmp_max_pt.y()) / 2.) - cloud_bbox_xyz->points[0].z, 2));
          if (distance > tmp_dis) {
            distance = tmp_dis;
            max_pt = tmp_max_pt;
            min_pt = tmp_min_pt;
        ...
        PointT obj_pt;
        obj_pt.x = xyz_centroid.x();
        obj_pt.y = xyz_centroid.y();
        obj_pt.z = xyz_centroid.z();
        publishObjectTf(obj_pt, object_pose.id);
        object_pose_array.detections.push_back(object_pose);
    ...
    void callback_BBoxPCL(const std::shared_ptr<vision_msgs::msg::Detection2DArray> bbox_msg,
      ...
      if (fast_shot_) processBBoxFastShot(bbox_msg, pcl_msg, img_msg);
      else            processBBoxClustering(bbox_msg, pcl_msg, img_msg);
    }
    ...
    void callback_RunCtr(const std::shared_ptr<std_srvs::srv::SetBool::Request> req, std::shared_ptr<std_srvs::srv::SetBool::Response> res) {
      if (req->data) {
        ...
        if (!sync_)       sync_       = std::make_shared<message_filters::Synchronizer<BBoxesCloudSyncPolicy>>(BBoxesCloudSyncPolicy(200), *sub_bboxes_, *sub_pcl_, *sub_img_);
  ...
  public:
    BboxTo3D() : Node("bbox_to_3d"), tfBuffer_(std::make_shared<rclcpp::Clock>(RCL_ROS_TIME)), tfListener_(tfBuffer_), tfBroadcaster_(this), sub_bboxes_(), sub_pcl_(), sub_img_(), sync_() {
