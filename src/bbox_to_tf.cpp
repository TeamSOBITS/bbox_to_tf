...
public:
    BboxToTF() : Node("bbox_to_tf"), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_), tf_broadcaster_(this) {
        ...
        // Get parameters
        this->get_parameter("base_frame_name", base_frame_name_);
        this->get_parameter("bbox_topic_name", bbox_topic_name_);
        this->get_parameter("cloud_topic_name", cloud_topic_name_);
        this->get_parameter("img_topic_name", img_topic_name_);
        this->get_parameter("execute_default", execute_flag_);
        this->get_parameter("cluster_tolerance", cluster_tolerance_);
        this->get_parameter("min_clusterSize", min_clusterSize_);
        this->get_parameter("max_clusterSize", max_clusterSize_);
        this->get_parameter("noise_point_cloud_range", noise_point_cloud_range_);

        // Initialize publishers and subscribers
        pub_obj_poses_ = this->create_publisher<sobits_msgs::msg::ObjectPoseArray>("object_poses", 10);
        pub_object_cloud_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("object_cloud", 1);

        // Initialize services
        run_ctr_srv_ = this->create_service<sobits_msgs::srv::RunCtrl>("run_ctr", std::bind(&BboxToTF::callback_RunCtr, this, std::placeholders::_1, std::placeholders::_2));

        // Initialize message filters and synchronizer
        sub_bboxes_ = std::make_shared<message_filters::Subscriber<sobits_msgs::msg::BoundingBoxes>>(this, bbox_topic_name_);
...
private:
    void callback_BBoxCloud(const sobits_msgs::msg::BoundingBoxes::SharedPtr bbox_msg, const sensor_msgs::msg::PointCloud2::SharedPtr cloud_msg, const sensor_msgs::msg::Image::SharedPtr img_msg) {
        RCLCPP_ERROR(this->get_logger(), "HELOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO111111");
        if (!execute_flag_) {
            return;
        }
        ...
        try {
            ...
            RCLCPP_ERROR(this->get_logger(), "Could NOT transform tf to: %s", ex.what());
            is_error_ = true;
        }
        RCLCPP_ERROR(this->get_logger(), "HELOOOO2222222 is_error_ %d", is_error_);
        ...
        if (!is_error_) {
            try {
                ...
                if (img_raw_.empty()) {
                    RCLCPP_ERROR(this->get_logger(), "Input_image error");
                    is_error_ = true;
                }
        ...
        if (!is_error_) {
            RCLCPP_ERROR(this->get_logger(), "HELOOOO2222222222222222222222222");
            ...
            for (int i = 0; i < bbox_msg->bounding_boxes.size(); ++i) {
                ...
                int index = img_raw_.cols * ((bbox.ymax + bbox.ymin) / 2) + (bbox.xmax + bbox.xmin) / 2;

                RCLCPP_ERROR(this->get_logger(), "OUTTTTTTTTTTT11111111111");
                RCLCPP_ERROR(this->get_logger(), "bbox.ymax: %d, bbox.ymin: %d, bbox.xmax: %d, bbox.xmin: %d", bbox.ymax, bbox.ymin, bbox.xmax, bbox.xmin);
                RCLCPP_ERROR(this->get_logger(), "img_raw_.cols: %d", img_raw_.cols);
                RCLCPP_ERROR(this->get_logger(), "index: %d", index);
                RCLCPP_ERROR(this->get_logger(), "cloud_transform->points.size(): %zu", cloud_transform->points.size());                
                
                if (index < 0 || index >= static_cast<int>(cloud_transform->points.size())) continue;

                RCLCPP_ERROR(this->get_logger(), "OUTTTTTTTTTTT2222222222222222222");
                ...
                for (int iy = 0; iy <= (bbox.ymax - bbox.ymin) / 2; iy++) {
                    RCLCPP_ERROR(this->get_logger(), "bbox.ymax - bbox.ymin %d", bbox.ymax - bbox.ymin);
                    for (int ix = 0; ix <= (bbox.xmax - bbox.xmin) / 2; ix++) {
                        RCLCPP_ERROR(this->get_logger(), "bbox.xmax - bbox.xmin %d", bbox.xmax - bbox.xmin);
                        if (ix == 0 && iy == 0) continue;
                        int index = img_raw_.cols * ((bbox.ymax + bbox.ymin) / 2 - iy) + (bbox.xmax + bbox.xmin) / 2 - ix;
                        ...
                        if (iy != 0) {
                            index = img_raw_.cols * ((bbox.ymax + bbox.ymin) / 2 + iy) + (bbox.xmax + bbox.xmin) / 2 - ix;
                            if (index >= 0 && index < cloud_transform->points.size() && checkNanInf(cloud_transform->points[index])) {
                                cloud_bbox->points.push_back(cloud_transform->points[index]);
                            }
                        }
                ...
                RCLCPP_ERROR(this->get_logger(), "HELOOOO3333333333333333333333333");
                ...
                RCLCPP_ERROR(this->get_logger(), "HELOOO44444444444444");
                ...
                // RCLCPP_ERROR(this->get_logger(), "HELOOOO22 %s", bbox.class_name);
            ...
            RCLCPP_INFO(this->get_logger(), "Publishing object poses");
    ...
    bool callback_RunCtr(const std::shared_ptr<sobits_msgs::srv::RunCtrl::Request> request, std::shared_ptr<sobits_msgs::srv::RunCtrl::Response> response) {
        RCLCPP_ERROR(this->get_logger(), "HELOOOO33333333333333333");
...
int main(int argc, char **argv) {
    ...
    rclcpp::spin(std::make_shared<BboxToTF>());
    rclcpp::shutdown();
    return 0;
}